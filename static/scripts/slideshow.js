...
const slides = [
    [
        ...
        a system of neural networks that can generate completely original digital images from a dataset of publically-available images. We understand the importance of originality 
        and privacy, so we allow any artist or photographer opt out their publically-available images from our GAN's training process. `
    ],
    [
        "Write Content <span class='purpText'>Effortlessly.</span>",
        "text-gen.png",
        `With Singularity, any user can us or Text-Generation Service to find inspiration, improve their own writing, or find answers to a vast range of questions.
        Our text-generation service utilizes a large language model(LLM) heavily trained on years of text data. The LLM is built in with a Transformer neural netweork structure, which
        ...
        LLMs are one of the most innovative and fast-growing apsects of artificial inteligence and here at Singularity, we aim to ensure a future where AI can serve as a tool for the progresion of society.`
    ],
    [
        "Programming <span class='purpText'>Made Easy.</span>",
        "code.png",
        `Writing code has never been easier with Singularity's code-generation service. The service is built on a large language model(LLM) specifically trained on publically available code.
